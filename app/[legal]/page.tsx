...
export default function Legal({ params }: { params: Promise<{ legal: string }> }) {
  ...
  useEffect(() => {
    params.then((resolvedParams) => {
      setLegalType(resolvedParams.legal);
    });
  }, [params]);
  
  const getTitle = () => {
    if (legalType === "privacy" || pathname?.includes("privacy")) return "Privacy Policy";
    if (legalType === "terms" || pathname?.includes("terms")) return "Terms of Service";
    if (legalType === "cookies" || pathname?.includes("cookies")) return "Cookie Policy";
    return "Legal";
  };
